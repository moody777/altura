...
const MyJobs: React.FC = () => {
  ...
  // Use DataContext for applications
  ...
  const filteredApplications = applications.filter(app => {
    const matchesSearch = (app.jobId || '').toLowerCase().includes(searchQuery.toLowerCase());
    const matchesStatus = filterStatus === 'all' || app.status === filterStatus;
    return matchesSearch && matchesStatus;
  });

  // Since we don't have recommendations in the new DataContext, we'll show empty state
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-3">
                            <h3 className="text-lg font-semibold text-white">Job Application</h3>
                            <span className={`px-2 py-1 rounded-full text-xs font-medium border flex items-center space-x-1 ${getStatusColor(application.status || 'pending')}`}>
                          ...
                          <p className="text-gray-300 text-sm mb-3">Application details will be shown here</p>
                        </div>

...
const Auth: React.FC = () => {
  ...
  const handleResendCode = async () => {
    try {
      // Resend the confirmation code by calling signUp again
      await signUp(formData.email, formData.password, formData.name);
    } catch (error) {
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-white mb-1">
              {authStep === 'signup' ? 'Create an account' : 
               authStep === 'confirm' ? 'Verify your email' : 'Welcome back'}
            </h1>
            <p className="text-gray-400">
              {authStep === 'signup' ? 'Already have an account?' : 
               authStep === 'confirm' ? `We sent a 6-digit code to ${formData.email}` : "Don't have an account?"}
              {authStep !== 'confirm' && (
                <button
                  onClick={() => {
                    if (authStep === 'signup') {
          ...
          <form onSubmit={handleSubmit} className="space-y-1">
            ...
            {authStep === 'confirm' && (
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="w-full px-4 py-3 bg-gray-700/50 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:ring-2 focus:ring-[#B8860B] focus:border-transparent transition-colors text-center text-2xl tracking-widest"
                  placeholder="000000"
                  maxLength={6}
                  required
                />
            ...
            >
              {loading ? 'Please wait...' : 
               ...
               authStep === 'confirm' ? 'Verify code' : 'Sign In'}
            </button>
          </form>
            ...
            <div className="mt-4">
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-4 bg-gray-800 text-gray-400">Or {authStep === 'signup' ? 'register' : 'sign in'} with</span>
                </div>
              </div>
